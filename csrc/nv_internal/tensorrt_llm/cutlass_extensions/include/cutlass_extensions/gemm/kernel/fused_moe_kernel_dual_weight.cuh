/*
 * Dual-weight fused MoE kernel for SM80 (placeholder copy of fused MoE wiring).
 * This currently uses the dual-weight traits/routine but does not aim for accuracy.
 */
...
#include <cutlass_extensions/gemm/kernel/dual_weight_fused_moe_kernel_routine.cuh>
...
struct Fused_Moe_Kernel_dual_weight_sm80 {
  ...
  using BaseKernelTraits =
      DualWeight_Fused_Moe_Kernel_traits_sm80<ElementInput, ElementWeight, ElementOutput, kMaxTileM,
  ...
  using BaseKernelTraits_m16 =
      DualWeight_Fused_Moe_Kernel_traits_sm80<ElementInput, ElementWeight, ElementOutput, 16, kTileN, kTileK,
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    return {{args.routine_args.ptr_input,
             args.routine_args.ptr_fc1_upper,
             args.routine_args.ptr_fc1_lower,
             args.routine_args.ptr_bias,
             args.routine_args.ptr_output,
             args.routine_args.total_tokens_including_expert,
...
#define ROUTINE_PATH(kTileM_size)                                                                 \
  {                                                                                               \
    ...
    using RoutineTraits =                                                                         \
        DualWeight_Fused_Moe_Kernel_routine_sm80<ElementInput, ElementWeight, ElementOutput, kTileM, kTileN, \
    ...
    while (problem_visitor.next_tile()) {
      ...
      if (residue_m > kMaxTileM / 2) {
        using RoutineTraits =
            DualWeight_Fused_Moe_Kernel_routine_sm80<ElementInput, ElementWeight, ElementOutput, kMaxTileM,
                                          kTileN, kTileK, kStages, activation_type>;
        RoutineTraits routine{};
