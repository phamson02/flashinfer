...
#include "./launchers/dual_weight_moe_gemm_tma_ws_launcher.h"
...
using tensorrt_llm::kernels::cutlass_kernels::DualWeightTmaWarpSpecializedGroupedGemmInput;
using EpilogueFusion = DualWeightTmaWarpSpecializedGroupedGemmInput::EpilogueFusion;
    ...
    auto select_dynamic_cga = [epilogue_schedule](auto dynamic_cga_t) {
      ...
      if constexpr ((!is_block_scaled || Arch::kMinComputeCapability == 103) &&
        ...
        auto func_map = std::array{
            &kernels::cutlass_kernels_oss::dual_weight_tma_warp_specialized_generic_moe_gemm_kernelLauncher<
        ...
        return &kernels::cutlass_kernels_oss::dual_weight_tma_warp_specialized_generic_moe_gemm_kernelLauncher<
            Arch, T, WeightType, OutputType, cutlass::epilogue::PtrArrayTmaWarpSpecialized,
            EpilogueTag, FUSION, TileShape, ClusterShape, is_wfp4afp8,
            decltype(dynamic_cga_t)::value, false, decltype(swap_ab_t)::value>;
      }
...
void dispatchDualWeightMoeGemmFinalDispatchTmaWarpSpecialized(
    DualWeightTmaWarpSpecializedGroupedGemmInput hopper_input, int num_experts,
  ...
  TLLM_CHECK_WITH_INFO(workspace_size || hopper_input.ptr_weight_2 != nullptr,
                       "Dual-weight TMA path requires lower-weight pointer array");
  ...
  if constexpr (Arch::kMinComputeCapability < 90) {
    ...
    if constexpr (is_wfp4afp8) {
      TLLM_CHECK_WITH_INFO(hopper_input.fpX_block_scaling_type ==
                               DualWeightTmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType::MXFPX,
    ...
    if constexpr (Arch::kMinComputeCapability >= 100 && Arch::kMinComputeCapability < 120) {
      ...
      // Select launcher based on swap_ab and use_custom_schedule from gemm_config
      auto select_launcher = [&](auto swap_ab_t, auto custom_sched_t) {
        return &kernels::cutlass_kernels_oss::dual_weight_tma_warp_specialized_generic_moe_gemm_kernelLauncher<
            Arch, T, WeightType, OutputType, EpilogueSchedule, EpilogueTag, FUSION,
            TileShape, ClusterShape, is_wfp4afp8, dynamic_cga, false,
            decltype(swap_ab_t)::value, decltype(custom_sched_t)::value>;
      };
      using common::ConstBool;
      auto selected_func =
          gemm_config.use_custom_schedule
              ? (hopper_input.swap_ab
                     ? select_launcher(ConstBool<true>{}, ConstBool<true>{})
                     : select_launcher(ConstBool<false>{}, ConstBool<true>{}))
              : (hopper_input.swap_ab
                     ? select_launcher(ConstBool<true>{}, ConstBool<false>{})
                     : select_launcher(ConstBool<false>{}, ConstBool<false>{}));
...
void dispatchDualWeightMoeGemmSelectClusterShapeTmaWarpSpecialized(
    DualWeightTmaWarpSpecializedGroupedGemmInput hopper_input, int num_experts,
    cutlass_extensions::CutlassGemmConfig gemm_config, int multi_processor_count,
    cudaStream_t stream, int* occupancy, size_t* workspace_size) {
  using namespace cute;
...
#define SHAPE_CASE(M, N, K)                                                                        \
  case cutlass_extensions::ClusterShape::ClusterShape_##M##x##N##x##K: {                           \
    ...
    if constexpr (are_tile_shapes_supported<Arch, TileShape, ClusterShape, T, WeightType>()) {     \
      dispatchDualWeightMoeGemmFinalDispatchTmaWarpSpecialized<Arch, T, WeightType, OutputType, EpilogueTag, \
...
void dispatchDualWeightMoeGemmSelectTileShapeTmaWarpSpecialized(
    DualWeightTmaWarpSpecializedGroupedGemmInput hopper_input, int num_experts,
    cutlass_extensions::CutlassGemmConfig gemm_config, int multi_processor_count,
    cudaStream_t stream, int* occupancy, size_t* workspace_size) {
  using namespace cute;
...
#define SHAPE_CASE(SMVERSION, M, N, K)                                                            \
  case cutlass_extensions::CutlassTileConfigSM##SMVERSION::CtaShape##M##x##N##x##K##B: {          \
    ...
    dispatchDualWeightMoeGemmSelectClusterShapeTmaWarpSpecialized<                                          \
...
size_t calcMaxWorkspaceSizeDualWeightTmaWarpSpecialized(
    int num_experts, cutlass_extensions::CutlassGemmConfig gemm_config, int multi_processor_count,
    DualWeightTmaWarpSpecializedGroupedGemmInput::FpXBlockScalingType fpX_block_scaling_type) {
  size_t count = 0;
  DualWeightTmaWarpSpecializedGroupedGemmInput input{};
  ...
  // template bloat
  dispatchDualWeightMoeGemmSelectTileShapeTmaWarpSpecialized<T, WeightType, OutputType,
