...
def _restore_autotuner_state(old_instance: AutoTuner, old_registered: bool) -> None:
...
def test_autotuner_default_path_and_exit_flush(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert (
            expected_path == tmp_path / ".cache" / "flashinfer" / "autotuner_cache.json"
        )
        assert registered, (
            "atexit.register should be called when persistence is enabled"
        )
